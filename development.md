...
- **`steady-py scan` on a single file fails by default.** Plain-text output isn't implemented for a single-file scan, so `steady-py scan my_notebook.ipynb` exits 2 with an error unless `--format json` is given. The most obvious first command a user tries shouldn't fail; implement the text form (the directory form already has one).
- **A folder scan reports a package named `---`.** Scanning a folder containing `tests/fixtures/unit/magic_sink.ipynb` lists `---` among the packages not resolvable via pip-freeze. Something in the magic-command harvesting turns a non-package token into an entry.
- **Cell 2's troubleshooting advice undoes the pin.** After a failure, `install()` suggests `!pip install <pkg>` with no version, which installs the newest release rather than the author's. It should suggest the exact version first (HELP.md already explains it that way).
- **The live-session recipe is clunky.** Getting the cells out of `steady_py.snapshot()` in a notebook takes `result.notebooks[0].cells.markdown` and `.code`. A helper that displays both cells (or a rich display of the result in IPython) would cut the README recipe to two lines, which matters for the non-engineer audience.
